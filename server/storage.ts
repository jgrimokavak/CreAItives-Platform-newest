...
import { db } from "./db";
import { eq, desc, asc, isNull, isNotNull, and, ilike, lt, lte, sql, or, not, inArray, ne, count } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async updateVideoJobState(id: string, updates: {
    nextPollAt?: Date;
    lastError?: string;
    attemptCount?: number;
  ...
  // Complete a video job with URL validation
  async completeVideoJob(id: string, completion: {
    url: string;
    thumbUrl?: string;
    fullUrl?: string;
    size?: number;
  ...
  async getVideosNeedingPoll(limit: number = 10): Promise<Video[]> {
    const currentEnv = process.env.REPLIT_DEPLOYMENT === '1' ? 'prod' : 'dev';
    const now = new Date();
  ...
  async getProcessingVideos(userId?: string): Promise<Video[]> {
    const currentEnv = process.env.REPLIT_DEPLOYMENT === '1' ? 'prod' : 'dev';
    const conditions = [
      eq(videos.environment, currentEnv),
      eq(videos.status, 'processing')
  ...
  // Enhanced update with state machine validation
  async updateVideoWithValidation(id: string, updates: Partial<Video>): Promise<Video | undefined> {
    // Get current video to validate state transition
    ...
    if (updates.status && updates.status !== currentVideo.status) {
      this.validateVideoStateTransition(currentVideo.status, updates.status, updates);
    }
  ...
  // Private method to validate state transitions
  private validateVideoStateTransition(
    ...
    // Validate required fields for each status
    switch (newStatus) {
      case 'processing':
        if (!updates.jobId) {
          throw new Error('jobId is required when setting status to processing');
        }
        break;
      case 'completed':
        if (!updates.url) {
          throw new Error('url is required when setting status to completed');
        }
        break;
    }
  }
