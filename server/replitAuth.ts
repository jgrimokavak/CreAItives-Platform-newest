...
async function upsertUser(
  ...
  // Check if user email is from @kavak.com domain before creating user
  if (!isKavakUser(claims["email"])) {
    throw new Error("Access restricted to @kavak.com users only");
  }
...
export async function setupAuth(app: Express) {
  ...
  const verify: VerifyFunction = async (
    ...
    try {
      ...
      // Redirect to home with error message for non-Kavak users
      if (error.message.includes("@kavak.com")) {
