...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Helper function to run JSON-based image edit job
  async function runJSONEditJob(jobId: string, data: any) {
    console.log(`Starting JSON edit job ${jobId} with model: ${data.modelKey}`);
    ...
    try {
      ...
      const { prompt, modelKey, images, mask, kavakStyle, aspect_ratio, seed, prompt_upsampling } = data;
      ...
      if (kavakStyle) {
        ...
        console.log(`Job ${jobId}: Using KAVAK style for edit`);
      }
      ...
      if (modelKey === "gpt-image-1") {
        // For GPT-Image-1, we use the existing edit logic but with base64 images
        console.log(`Job ${jobId}: Processing with OpenAI GPT-Image-1`);
        
        // Convert base64 images and call OpenAI edit API
        ...
        // Process first image for editing
        ...
        const tempPath = path.join(tempDir, `edit_${jobId}.png`);
        fs.writeFileSync(tempPath, imageBuffer);
        
        const uploadable = await toUploadable(tempPath);
        editParams.image = uploadable;
        
        // Add mask if provided
        if (mask) {
          const maskData = mask.replace(/^data:image\/[a-z]+;base64,/, '');
          const maskBuffer = Buffer.from(maskData, 'base64');
          const maskPath = path.join(tempDir, `mask_${jobId}.png`);
          fs.writeFileSync(maskPath, maskBuffer);
          const maskUploadable = await toUploadable(maskPath);
          editParams.mask = maskUploadable;
        }
        
        console.log(`Job ${jobId}: Calling OpenAI image edit`);
        
        // Call OpenAI edit API
        const response = await openai.images.edit(editParams);
        
        // Process response similar to generation
        generatedImages = await Promise.all(response.data.map(async (imageData: any, index: number) => {
          let imageUrl: string;
          let fullUrl: string;
          let thumbUrl: string;
          ...
          if (imageData.url) {
            const result = await downloadAndSaveImage(
              ...
              prompt,
              modelKey,
              `img_${createFileSafeNameFromPrompt(prompt)}_${index}`
            );
          ...
          const newImage: GeneratedImage = {
            id: `img_${createFileSafeNameFromPrompt(prompt)}_${index}`,
            url: imageUrl,
            fullUrl: fullUrl,
            thumbUrl: thumbUrl,
            prompt: prompt,
      ...
      else if (modelKey === "flux-kontext-max") {
        // Handle Flux-Kontext-Max editing via Replicate
        ...
        const inputs = {
          ...
          safety_tolerance: 2,
          output_format: "png",
          image: images[0], // Flux-Fill takes a single image
        };
        ...
        console.log(`Job ${jobId}: Sending Flux-Kontext-Max edit request`);
        
        // Call the Replicate generation function
        generatedImages = await generateWithReplicate(modelKey, inputs);
        console.log(`Job ${jobId}: Generated ${generatedImages.length} images with Flux-Kontext-Max`);
      }
      else {
        throw new Error(`Unsupported model for editing: ${modelKey}`);
      }
      ...
      broadcastToClients("imageCreated", { images: generatedImages, jobId });
  ...
  // API endpoint for async image editing (using job queue) - JSON version
  app.post("/api/edit-image", isAuthenticated, async (req, res) => {
    try {
      // Check if it's JSON or multipart
      const isJSON = req.headers['content-type']?.includes('application/json');
      
      if (isJSON) {
        // Handle JSON request (from new EditForm)
        console.log("Processing JSON edit request:", { modelKey: req.body.modelKey, images: req.body.images?.length });
        
        // Validate that required fields are present
        if (!req.body.prompt || !req.body.images || !Array.isArray(req.body.images) || req.body.images.length === 0) {
          return res.status(400).json({ message: "Prompt and images array are required" });
        }
        
        if (req.body.images.length > 16) {
        ...
        console.log(`Created JSON edit job ${jobId} with model: ${req.body.modelKey}`);
        
        // Start the job processing in the background
        process.nextTick(() => runJSONEditJob(jobId, req.body));
        
        // Return the job ID immediately
        return res.status(202).json({ jobId });
      } else {
        // Handle multipart form data (use existing runEditJob function)
        const prompt = req.body.prompt;
        if (!prompt) {
          return res.status(400).json({ message: "Prompt is required" });
        }
        
        // Create a new job for multipart edit
        const jobId = crypto.randomUUID();
        jobs.set(jobId, { 
          id: jobId, 
          status: "pending",
        ...
        console.log(`Created multipart edit job ${jobId}`);
        
        // Start the job processing in the background using existing runEditJob
      ...
      res.status(500).json({ message: error.message || "Failed to process edit request" });
    }
  });

  // Legacy multipart handler (kept for backward compatibility)
  app.post("/api/edit-image-legacy", 
    upload.fields([
