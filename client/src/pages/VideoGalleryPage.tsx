...
export default function VideoGalleryPage() {
  ...
  const duplicateProjectMutation = useMutation({
    mutationFn: (projectId: string) => 
      apiRequest(`/api/projects/${projectId}/duplicate`, { method: 'POST', body: {} }),
    onSuccess: (data) => {
      toast({
  ...
  const deleteProjectMutation = useMutation({
    mutationFn: ({ projectId, deleteVideos }: { projectId: string; deleteVideos: boolean }) => 
  ...
  const reorderProjectsMutation = useMutation({
    mutationFn: (projectIds: string[]) =>
      apiRequest('/api/projects/reorder', { method: 'POST', body: { projectIds } }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects'] });
    },
    onError: (error) => {
  ...
  const handleDeleteConfirm = () => {
    ...
    if (project && deleteProjectName === project.name) {
      deleteProjectMutation.mutate({ 
        projectId: deleteConfirmProject!, 
        deleteVideos 
      });
    }
  };
