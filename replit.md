...
- **Phase 1 & 2 Static Deployment Split (August 18, 2025)**: Successfully implemented split architecture preparation. Phase 1 created bootstrap utility for API base URL patching (`client/src/bootstrap/patchFetchWithPublicApiBaseUrl.ts`) and CORS middleware (`server/middleware/enforceCorsForStaticOrigin.ts`) while preserving current behavior. Phase 2 completed the split by enabling CORS allowlist, removing server-side SPA hosting in production, and creating API-only mode with 404 responses for non-API paths. Development workflow preserved via Vite. Ready for production deployment with static frontend (zero compute) and API backend (reduced load).
